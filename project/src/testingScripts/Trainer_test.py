...
class Classifier:
    ...
    def train(self, 
        ...
        save_steps = 1 #len(train_dataset) // (per_gpu_train_batch_size * gradient_accumulation_steps* self.n_gpu)
    ...
    def predict(self, per_gpu_eval_batch_size):
        ...
        validation_dataset = self.data_loader.load_dataset('validation')
        validation_dataset.reset_index(inplace=True)
        validation_dataset.index = range(1, len(validation_dataset) + 1)

        validation_df = validation_dataset['label']

        if not os.path.exists(os.path.join('validation_sets', self.pretrained_model_name_or_path)):
          os.makedirs(os.path.join('validation_sets', self.pretrained_model_name_or_path))

        validation_df.to_csv(os.path.join('validation_sets', self.pretrained_model_name_or_path, 'validation_set'), header=False)
